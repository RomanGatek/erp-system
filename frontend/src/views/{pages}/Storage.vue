    ...
    <div class="bg-white p-6 rounded-2xl shadow-lg ring-1 ring-gray-100">
      ...
      <template v-else>
        <div class="max-h-[500px] overflow-y-auto">
          <DataTable :headers="tableHeaders" :items="inventoryStore.paginateItems" :sort-by="inventoryStore.setSorting"
            :sorting="inventoryStore.sorting" :on-edit="openEditModal" :on-delete="deleteItem">
            <template #row="{ item }">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-gray-600">
                {{ item.stockedAmount.toString().length < 1 ? 0 : item.stockedAmount }} </td>
    ...
    <Modal :show="isAddModalOpen" title="Add New Item" @close="cancelAdd">
      <div class="space-y-3">
        ...
        <div class="flex justify-end space-x-3 pt-2">
          ...
          <BaseButton type="primary" class="text-sm! font-bold flex!" @click="addItem">
            Add Item
          </BaseButton>
        </div>
      </div>
    ...
    <Modal :show="isEditModalOpen" title="Edit Item" @close="cancelEdit">
      <div class="space-y-3">
        ...
        <div class="flex justify-between pt-2">
          <BaseButton type="error" class="text-sm! font-bold flex!" @click.stop="cancelEdit">
            Cancel
          </BaseButton>
          <BaseButton type="primary" class="text-sm! font-bold flex!" @click="updateItem">
            Update Item
          </BaseButton>
        </div>
      </div>
