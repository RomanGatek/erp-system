        ...
        <div class="flex items-center space-x-4">
          <div class="flex space-x-2">
            <button v-for="filter in filters" :key="filter.value" @click="currentFilter = filter.value"
              class="px-3 py-1.5 text-sm rounded-lg transition" :class="currentFilter === filter.value
          ...
          <template #row="{ item }">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
              ...
              <button @click.stop="openOrderEdit(item)"
                class="text-green-600 hover:text-green-900 p-1 rounded hover:bg-blue-50" title="Edit Order">
                <svg fill="none" stroke="currentColor" x="0px" y="0px" class="w-5 h-5" viewBox="0 0 24 24">
                  <path
              ...
              <button @click.stop="confirmDelete(item)"
                class="text-red-600 hover:text-red-900 p-1 rounded hover:bg-red-50" title="Delete Order">
                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div v-if="selectedOrder.status === 'PENDING'" class="space-y-4 pt-4 border-t border-gray-200">
          ...
          <div class="bg-white rounded-xl border border-gray-200">
            ...
            <div class="flex justify-end gap-2 px-4 py-3 bg-gray-50 rounded-b-xl">
              <BaseButton :disabled="workflowStore.loading" type="error" class="text-sm! font-bold flex!"
                @click="rejectOrder">
                Reject Order
              </BaseButton>
              <BaseButton :disabled="workflowStore.loading" type="primary" class="text-sm! font-bold flex!"
                @click="approveOrder">
                Confirm Order
              </BaseButton>
            </div>
        ...
        <div v-if="selectedOrder.status !== 'PENDING'" class="space-y-4 pt-4 border-t border-gray-200">
          ...
          <div class="bg-gray-50 rounded-xl divide-y divide-gray-200">
            ...
            <div v-if="selectedOrder.comment" class="p-4 space-y-2">
              ...
              <p class="mt-1 text-sm text-gray-700 bg-white p-3 rounded-lg border border-gray-200 whitespace-pre-wrap">
                {{ parsedComment }}
              </p>
            </div>
          </div>
